...
const editEquipment = () => {
  if (!selectedEquipment.value.type || selectedEquipment.value.type.length < 2) {
    alert('Type must be at least 2 characters long.');
    return;
  }
  if (!selectedEquipment.value.model || selectedEquipment.value.model.length < 2) {
    alert('Model must be at least 2 characters long.');
    return;
  }
  if (!selectedEquipment.value.serialNumber) {
    alert('Serial Number is required.');
    return;
  }
  if (!selectedEquipment.value.assignedTo) {
